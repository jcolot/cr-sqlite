...
export class AccessHandlePoolVFS extends VFS.Base {
  ...
  xRead(fileId, pData, iOffset) {
    ...
    // pData is a view onto emscripten's WASM heap, whose backing ArrayBuffer is
    // *resizable* when built with ALLOW_MEMORY_GROWTH. OPFS FileSystemSyncAccessHandle
    // rejects resizable-backed views ("must not be resizable"), so read into a fresh
    // non-resizable scratch buffer and copy the bytes into pData.
    const scratch = new Uint8Array(pData.byteLength);
    const nBytes = file.accessHandle.read(scratch, { at: HEADER_OFFSET_DATA + iOffset });
    new Uint8Array(pData.buffer, pData.byteOffset, pData.byteLength).set(scratch);
  ...
  xWrite(fileId, pData, iOffset) {
    ...
    // See xRead: copy out of the resizable heap into a non-resizable buffer before
    // handing it to the OPFS sync write.
    const scratch = new Uint8Array(pData.byteLength);
    scratch.set(new Uint8Array(pData.buffer, pData.byteOffset, pData.byteLength));
    const nBytes = file.accessHandle.write(scratch, { at: HEADER_OFFSET_DATA + iOffset });
